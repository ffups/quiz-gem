    ...
    try {
        ...
        document.getElementById('submit-button').addEventListener('click', () => {
            ...
            if (isCorrect) {
                ...
                showFeedback(`❌ Incorrect. The correct answer is: ${data.results[currentIndex].correct_answer}`, false);
            }
        ...
        document.getElementById('data-container').innerHTML = '<p>Failed to load questions. Please try again later.</p>';
    }
});
...
function displayQuestion(results, index, answeredQuestions) {
    ...
    questionText.textContent = `Q${index + 1}: ${decodeHTMLEntities(item.question)}`;
    questionElement.appendChild(questionText);

    const answersList = document.createElement('ul');

    // Combine correct and incorrect answers
    const answers = [item.correct_answer, ...item.incorrect_answers].map(answer => decodeHTMLEntities(answer));
